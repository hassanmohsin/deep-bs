...
class Grid(object):
    ...
    def resample(self, shape, center, spacing=None, bounds_error=False, fill_value=0):
        ...
        grid.shape = shape
        grid.center = center
        points = [np.arange(self.origin[i], self.origin[i] + self.spacing[i] * self.shape[i], self.spacing[i]) for i in
                  range(self.ndim)]
        g = interpolate.RegularGridInterpolator(points, self.ndelements, bounds_error=bounds_error,
